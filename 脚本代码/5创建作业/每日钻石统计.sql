...
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]]    Script Date: 2017/6/14 17:18:41 ******/
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=@JOBNAME, 
		...
		@command=N'EXEC NET_PJ_DiamondStatistics', 
		@database_name=N'WHQJTreasureDB', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=@JOBNAME, 
		...
		@schedule_uid=N'5294cdda-b7da-4e62-ab86-29b1e53136df'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
