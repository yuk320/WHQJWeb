...
SET @JOBNAME = N'每日排行统计'

/****** Object:  Job [每日排行统计]    Script Date: 2017/6/15 9:53:36 ******/
BEGIN TRANSACTION
...
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [每日排行统计]    Script Date: 2017/6/15 9:53:36 ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=@JOBNAME, 
		...
		@command=N'EXEC NET_PJ_RankingStatistics', 
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=@JOBNAME, 
		...
		@schedule_uid=N'99a7f3b3-89cd-459d-a684-19e202646ec0'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
