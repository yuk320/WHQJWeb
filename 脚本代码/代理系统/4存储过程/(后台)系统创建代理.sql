...
BEGIN
	...
	-- 查询代理重复信息
	IF EXISTS(SELECT AgentID FROM AgentInfo WITH(NOLOCK) WHERE UserID=@dwUserID)
	BEGIN
	...
	IF @@ERROR=0 
	BEGIN
		-- 更新用户信息，设置代理标识，取消原推广关系
		UPDATE WHQJAccountsDB.DBO.AccountsInfo SET SpreaderID=0,AgentID=@NewAgentID WHERE UserID = @dwUserID
		...
		BEGIN
			UPDATE WHQJAccountsDB.DBO.AccountsInfo SET SpreaderID = 0 WHERE SpreaderID = @UserID
		END
		ELSE 
		BEGIN
