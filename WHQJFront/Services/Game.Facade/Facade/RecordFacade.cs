...
namespace Game.Facade
{
    ...
    public class RecordFacade
    {
        ...
        {
            DataRow dr =recordData.GetOne($"SELECT COUNT(1) AS [Count] FROM (SELECT TargetUserID FROM RecordPresentCurrency(NOLOCK) WHERE SourceUserID={userid} AND TargetUserID NOT IN (SELECT UserID FROM WHQJAccountsDBLink.WHQJAccountsDB.dbo.AccountsAgentInfo) GROUP BY TargetUserID) AS a ");
